...
def _rate_limit(ip: str) -> None:
    now = time.time()
    hits = ip_request_times.setdefault(ip, [])
    # 清理窗口外记录
    ip_request_times[ip] = [t for t in hits if now - t <= RATE_LIMIT_WINDOW]
...
if __name__ == "__main__":
    # 便于本地启动：python app/server.py 或 python modules/01-fastapi-service/app/server.py
